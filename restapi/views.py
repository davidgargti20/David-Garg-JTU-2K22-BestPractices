...
def get_user_ids(body, type: str):
    ...
    user_ids: list[int] = []
    if (
        body.get(type, None) is not None
        and body[type].get("user_ids", None) is not None
    ):
...
def balance(request):
    ...
    for expense in expenses:
        ...
        for eb in expense_balances:
            ...
            if to_user == user.id:
                final_balance[from_user] = (
                    final_balance.get(from_user, 0) + eb["amount"]
                )
...
def normalize(expense):
    ...
    for user_balance in user_balances:
        dues[user_balance.user] = (
            dues.get(user_balance.user, 0)
            + user_balance.amount_lent
...
class group_view_set(ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if self.request.query_params.get("q", None) is not None:
            ...
            logging.info(
                f"Returned users with name containing {self.request.query_params.get('q', None)}"
            )
        else:
    ...
    def balances(self, _request, pk=None):
        ...
        dues: list[float] = [
            (k, v) for k, v in sorted(dues.items(), key=lambda item: item[1])
        ]
        start: int = 0
        end: int = len(dues) - 1
        balances: list = []
...
class expenses_view_set(ModelViewSet):
    ...
    def get_queryset(self):
        user = self.request.user
        if self.request.query_params.get("q", None) is not None:
            expenses = Expenses.objects.filter(users__in=user.expenses.all()).filter(
                description__icontains=self.request.query_params.get("q", None)
            )
        else:
            expenses = Expenses.objects.filter(users__in=user.expenses.all())
        return expenses
